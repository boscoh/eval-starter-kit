...
1. Start the web server (defaults to consultant evaluations):
   ```bash
   uv run server.py
   ```

2. Or start with engineer evaluations:
   ```bash
   uv run server.py --evals-dir evals-engineer
   ```
...
- **Queries** - Define input/output test cases
- **Prompts** - Edit system prompts
- **Graph** - Visualize performance metrics across models

### 4. Running Evaluations via CLI

The project has two separate evaluation directories:
- `evals-consultant/` - For consultant candidate evaluations (default)
- `evals-engineer/` - For engineer candidate evaluations

**Run all configs in default directory (evals-consultant):**

```bash
uv run runner.py
```

**Run all configs from engineer directory:**

```bash
uv run runner.py --evals-dir evals-engineer
```

**Run a specific config:**

```bash
uv run runner.py evals-consultant/runs/consultant-openai-gpt-4o.yaml
```

**Specify evaluation directory for specific config:**

```bash
uv run runner.py evals-engineer/runs/engineer-openai-gpt-4o.yaml --evals-dir evals-engineer
```

## Run Configuration

Evaluation configurations are defined in YAML files in the `evals-*/runs/` directories.

Example `evals-consultant/runs/consultant-openai-gpt-4o.yaml`:
...
The project uses separate evaluation directories to organize different types of candidate assessments:

### evals-consultant/ (Default)
Contains all evaluation configurations, queries, prompts, and results for consultant candidates. This is the default directory used by both `runner.py` and `server.py`.

### evals-engineer/
Contains all evaluation configurations for engineer candidates. Use the `--evals-dir evals-engineer` flag to work with this directory.

### Directory Layout
Each evaluation directory contains:
- `prompts/` - System prompts used to configure LLM behavior
- `queries/` - Test cases with input and expected output
- `runs/` - Evaluation configurations combining prompts, queries, models, and evaluators
- `results/` - Evaluation results generated by the runner

### Switching Between Directories

**CLI:**
```bash
# Use consultant directory (default)
uv run runner.py

# Use engineer directory
uv run runner.py --evals-dir evals-engineer
```

**Web Server:**
```bash
# Start with consultant directory (default)
uv run server.py

# Start with engineer directory
uv run server.py --evals-dir evals-engineer --port 8001
```

**Note:** The web server loads one evaluation directory at startup. To work with a different directory, restart the server with the appropriate `--evals-dir` flag.
...
├── evals-consultant/            # Consultant evaluation directory (default)
...
│   │   ├── consultant-openai-gpt-4o.yaml
│   │   ├── consultant-bedrock-nova-pro.yaml
│   │   └── ...
│   └── runs/                    # Run configurations (what to evaluate)
│       ├── consultant-openai-gpt-4o.yaml
│       ├── consultant-bedrock-nova-pro.yaml
│       └── ...
├── evals-engineer/              # Engineer evaluation directory
│   ├── prompts/                 # Reusable system prompts
│   │   ├── candidate-skills.txt
│   │   └── candidate-summary.txt
│   ├── queries/                 # Prompt/query YAMLs used by runs
│   │   └── engineer.yaml
│   ├── results/                 # Evaluation results (written by runner)
│   │   ├── engineer-openai-gpt-4o.yaml
│   │   ├── engineer-bedrock-nova-pro.yaml
│   │   └── ...
│   └── runs/                    # Run configurations (what to evaluate)
│       ├── engineer-openai-gpt-4o.yaml
│       ├── engineer-bedrock-nova-pro.yaml
